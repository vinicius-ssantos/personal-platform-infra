...
| `vos-studio-mcp` | `vinicius-ssantos/vos-studio-mcp` | VOS Studio MCP server | unknown | `ghcr.io/vinicius-ssantos/vos-studio-mcp:latest` configured | yes, profile `vos` | yes, `VOS_STUDIO_MCP_IMAGE` | missing | missing | yes | wired | none | Exposes MCP runtime on port `8000` inside the container. |
| `vos-studio-bff` | `vinicius-ssantos/vos-studio-bff` | BFF for VOS Studio flows | unknown | `ghcr.io/vinicius-ssantos/vos-studio-bff:latest` configured | yes, profile `vos` | yes, `VOS_STUDIO_BFF_IMAGE` | missing | missing | yes | wired | none | Depends on `vos-studio-mcp` at `http://vos-studio-mcp:8000`. |
