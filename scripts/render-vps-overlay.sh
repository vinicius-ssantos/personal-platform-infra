...
# Validate formats to prevent sed injection via special characters.
...
rendered="$(printf '%s\n' "$raw" | sed "s/__VPS_DOMAIN__/${VPS_DOMAIN}/g")"
...
if printf '%s\n' "$rendered" | grep -q '__R2_ENDPOINT__'; then
  ...
  rendered="$(printf '%s\n' "$rendered" | sed "s|__R2_ENDPOINT__|${R2_ENDPOINT}|g")"
fi
...
if printf '%s\n' "$rendered" | grep -q '__R2_BUCKET__'; then
  ...
  rendered="$(printf '%s\n' "$rendered" | sed "s/__R2_BUCKET__/${R2_BUCKET}/g")"
fi
