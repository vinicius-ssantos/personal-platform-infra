...
- **`.env` corrompido (encoding) bloqueia `compose-up`, `k3d-secrets` e `env-rotate-tokens`** — `scripts/check-env-encoding.sh` detecta bytes de mojibake (re-encode UTF-8 como Latin-1/CP1252, tipicamente por `[System.IO.File]::ReadAllText`/`-replace`/`WriteAllText` do PowerShell) e linhas absurdamente longas (>4000 chars), e falha alto em vez de deixar subir serviço ou secret corrompido. Se disparar, restaure de `.env.bak.*` (gerado por `just env-backup`) e corrija a linha afetada com a ferramenta Edit, nunca com manipulação de string do PowerShell.
